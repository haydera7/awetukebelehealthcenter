...
import { X, FileText, Pill, Activity, Loader2, Info, UploadCloud } from 'lucide-react';
import { useData } from '../../contexts/DataContext';
import { useAuth } from '../../contexts/AuthContext';

export default function AddRecordModal() {
  const { addMedicalRecord, closeAddRecordModal, selectedPatientId, selectedVisitId, visits, patients } = useData();
  const { user } = useAuth();
            ...
            <div className="form-group">
              <label className="form-label">
                {formData.type === 'Diagnosis' ? 'Diagnosis Title' : 
                 formData.type === 'Prescription' ? 'Medication Name' : 'Test Name'}
              </label>
              <input 
                name="title"
                type="text" 
            ...
            <div className="form-group">
              <label className="form-label">
                {formData.type === 'Diagnosis' ? 'Clinical Notes' : 
                 formData.type === 'Prescription' ? 'Dosage Instructions' : 'Result Summary'}
              </label>
              <textarea 
                name="notes"
                className="form-control" 
